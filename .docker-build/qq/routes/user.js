...
const user = {
  ...
  '/setCookie': ({req, res, globalCookie}) => {
    ...
    const userCookie = parseCookie(data);
    if (!userCookie.uin) {
      return res.send({
        result: 500,
        errMsg: 'cookie 里缺少 uin，请从 y.qq.com 的请求头复制完整 cookie',
      });
    }

    allCookies[userCookie.uin] = userCookie;
    jsonFile.writeFile(dataFile('allCookies.json'), allCookies);

    // uin 与本站 QQ 号一致时，登记为服务端全局登录态
    if (String(userCookie.uin) === String(global.QQ)) {
      globalCookie.updateUserCookie(userCookie);
      jsonFile.writeFile(dataFile('cookie.json'), userCookie);
    }
